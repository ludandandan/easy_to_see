...
def extract_patches(img, patch_size=(3, 3), stride=(1, 1)):
    ...
    patches = img.unfold(2, patch_size[0], stride[0]).unfold(3, patch_size[1], stride[1]) #[20,512,26,26,3,3] 
    patches = patches.contiguous().view(img.shape[0], img.shape[1], -1, patch_size[0], patch_size[1])#[20,512,676,3,3]
    patches = patches.transpose(1, 2)#[20,676,512,3,3]
    return patches
