...
def feature_mrf_loss_func(x, y, weight_img, vgg_model=None, layer=[], match_img_vgg=[], topk=1):
    ...
    x_feat = vgg_model(x, layer)
    y_feat = vgg_model(y, layer)
    weight_feat = vgg_model(weight_img, layer) #ld add
    # ld add
    for i in range(len(weight_feat)):
        max_v = weight_feat[i].max()
        min_v = weight_feat[i].min()
        weight_feat[i] = weight_feat[i]/(max_v - min_v)
    ...
    if len(match_img_vgg) == 0:
        ...
        for pred, gt, weight, match0, match1 in zip(x_feat, y_feat, weight_feat, match_img_feat[0], match_img_feat[1]):
            loss += mrf_crit(pred, gt, weight, [match0, match1])
    return loss
...
class MRFLoss(nn.Module):
    ...
    def best_topk_match(self, x1, x2):
        ...
        for i in range(x1.size(0)):
            ...
            for j in range(self.topk):
                ...
                cosine_dist = dist_func(x1[i].unsqueeze(0))  # [1,2916,54,54]
                max_value, max_idx = torch.max(cosine_dist, dim=1, keepdim=False)  # max_value: [1,54,54], max_idx: [1,54,54]
                tmp_value.append(max_value) # topk
                tmp_idx.append(max_idx)
            topk_value  = torch.stack(tmp_value)
            _, k_idx    = torch.max(topk_value, dim=0, keepdim=False) #[1,54,54], k_idx.max=4, k_dix.min=0
            spatial_idx = torch.stack(tmp_idx) # [5,1,54,54], .max=2915, .min=0
            k_match.append(k_idx.squeeze().view(-1).data) # the nearnest in the topk images, 
            spatial_match.append(spatial_idx.squeeze(1).view(spatial_idx.shape[0], -1).data) # the nearest in the image
        return torch.stack(k_match), torch.stack(spatial_match) # k_match: [4,2916](4 is the batchsize) max:4, spatial_match:[4,5,2916],(4 is the batchsize, 5 is topk). max:2915
    ...
    def forward(self, pred_style, target_style, weight, match=[]):
        """
        ...
        target_style_patches = extract_patches(target_style, self.patch_size, self.filter_patch_stride) #[20,2916,256,3,3]
        pred_style_patches = extract_patches(pred_style, self.patch_size, self.compare_stride) #[4,2916,256,3,3]
        weight_patches = extract_patches(weight, self.patch_size, self.compare_stride) #[4,2916,256,3,3]
        ...
        self.k_best_match = k_best_match # [4,2916] (4 is the batchsize), max = 4
        self.spatial_best_match = spatial_best_match #[4,5,2916] (4 is the batchsize, 5 is topk), max=2915

        pred_shape = pred_style_patches.size() # [4,2916,256,3,3]
        new_topk_target_style_patches = tensorToVar(torch.zeros(pred_shape[0]*self.topk,#[20,2916,256,3,3]
                                                pred_shape[1], pred_shape[2], pred_shape[3], pred_shape[4])) 
        spatial_best_match = spatial_best_match.view(pred_shape[0]*self.topk, -1) # [20,2916]
        for i in range(pred_shape[0]*self.topk): # target_style_patches:[20,2916,256,3,3], put the nearest patch in new_topk_target_style_patches
            new_topk_target_style_patches[i] = target_style_patches[[i], spatial_best_match[i]]
        new_topk_target_style_patches = new_topk_target_style_patches.view(pred_shape[0], self.topk, #[4,5,2916,256,3,3]
                                                            pred_shape[1], pred_shape[2], pred_shape[3], pred_shape[4]) 
        new_target_style_patches = tensorToVar(torch.zeros(pred_shape)) # top1, [4,2916,256,3,3]
        for i in range(k_best_match.shape[0]):
            for j in range(k_best_match.shape[1]):
                new_target_style_patches[i, j] = new_topk_target_style_patches[i, k_best_match[i, j], j] # put the nearest patch [256,3,3] to the new target_style_patches
        self.new_style_feature = new_target_style_patches #[4,2916,256,3,3]        
        #loss1 = self.crit(pred_style_patches*weight_patches, new_target_style_patches*weight_patches) # MSE LOSS
        loss = self.crit(pred_style_patches, new_target_style_patches)  # MSE Loss
        #return_loss = loss + loss1
        return loss 
